...
static std::unordered_map<std::string, CEccoServerCommand*> s_mapRegistedCommandMap;

static void EngineCallWrapper() {
    std::string cmd = CMD_ARGV(0);
    auto it = s_mapRegistedCommandMap.find(cmd.substr(ECCO_CMD_PREFIX_LEN));
    if (it != s_mapRegistedCommandMap.end())
        it->second->Call();
}

void RegistServerCommandToEngine() {
    for (auto& cmd : s_mapRegistedCommandMap) {
