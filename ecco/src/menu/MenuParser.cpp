...
static ecco_parser_item_t* CreateParserItem(ecco_parser_item_s* pParent, const std::string& id, CEccoScriptItem* pItem = nullptr) {
    auto* pParserItem = new ecco_parser_item_t();
    pParserItem->szId = id;
...
T* CreateExecutor() {
    auto* executor = new T();
    executor->m_iIndex = g_aryEccoMenuExecutors.size();
    g_aryEccoMenuExecutors.push_back(executor);
    return executor;
}
...
static void ParseMenu(CEccoTextMenuExecutor* pParentExecutor, ecco_parser_item_t* pParserItem, bool root = false) {
    ...
    std::sort(pParserItem->aryChild.begin(), pParserItem->aryChild.end(),
        [](ecco_parser_item_t* a, ecco_parser_item_t* b) { return a->szId < b->szId; });
    auto addBackItem = [&]() {
        if (root) {
            pMenu->AddItem(nullptr);
            return;
        }
        auto* pBackExecutor = CreateExecutor<CEccoBackExecutor>();
        pBackExecutor->m_szId = "ecco_menu_back";
        pBackExecutor->m_pParentExecutor = pParentExecutor->m_pParent;
        pMenu->AddItem(pBackExecutor);
    };

    auto addPrevPageItem = [&]() {
        if (page > 0) {
            auto* pPrevExecutor = CreateExecutor<CEccoBackExecutor>();
            pPrevExecutor->m_szId = "ecco_menu_prev_page";
            pPrevExecutor->m_pParentExecutor = pMenu->m_pParent;
            pMenu->AddItem(pPrevExecutor);
        }
        else {
            pMenu->AddItem(nullptr);
        }
    ...
    auto addExitItem = [&]() {
        auto* pExitExecutor = CreateExecutor<CBaseEccoExecutor>();
        pExitExecutor->m_szId = "ecco_menu_exit";
        pMenu->AddItem(pExitExecutor);
    };
    for (auto& child : pParserItem->aryChild) {
        if (page > 0 && counter == 0) {
            auto* pNextPageMenu = CreateExecutor<CEccoTextMenuExecutor>();
        ...
        if (child->pScriptItem) {
            auto* pExecutor = CreateExecutor<CEccoScriptExecutor>();
            ...
            auto* pExecutor = CreateExecutor<CEccoTextMenuExecutor>();
        ...
        counter++;
        if (pParserItem->aryChild.size() > MAX_ITEM_NO_PAGE &&
            counter % (MAX_ITEMS_PER_PAGE - 1) == 0) {
            addBackItem();
            addPrevPageItem();
            page++;
            counter = 0;
    ...
    const int itemsPerPage = MAX_ITEMS_PER_PAGE - 1;
    const int result = pParserItem->aryChild.size() % itemsPerPage;
    if (page > 0) {
        if (result != 0) {
            for (int i = result; i < itemsPerPage; i++) {
                pMenu->AddItem(nullptr);
            }
            addBackItem();
            addPrevPageItem();
